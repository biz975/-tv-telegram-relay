...
# MODE: Locker, aber ≥80% – Safe-Entry optional (mit Volumen-Bestätigung), Volumen-Spike Bonus, ADX≥20 Pflichtfilter, PROB_MIN=80
...
# ====== Analyse & Scan ======
TF_TRIGGER = "5m"                  # Signal-Zeitrahmen (Bias/Momentum/Vol/RSI/EMAs)
TF_FILTERS = ["15m", "1h"]         # Trendfilter: 15m & 1h müssen gleichgerichtet sein
LOOKBACK = 500
SCAN_INTERVAL_S = 15 * 60         # alle 15 Minuten

# ====== Safe-Entry (Pullback in Fib-Zone auf 15m) ======
SAFE_ENTRY_REQUIRED = False       # Safe-Entry ist KEIN KO-Kriterium (optional)
SAFE_ENTRY_TF = "15m"             # Fib-/Impuls-Check auf 15m
PIVOT_LEFT_TRIG = 2               # Pivot-Punkte für Impuls-Erkennung
PIVOT_RIGHT_TRIG = 2
FIB_TOL_PCT = 40 / 100.0          # ±0.40% Toleranz rund um 0.5–0.618

# Volumen-Bestätigung direkt am Safe-Entry (15m)
ENTRY_VOL_FACTOR = 1.20           # Volumen > MA20 × Faktor (z.B. 1.20)
REQUIRE_ENTRY_VOL = True          # Entry nur mit Volumen-Bestätigung (Qualitätsanker)
...
MIN_STRENGTH = 3                  # min. Anzahl Swings für „starkes“ Level
TP2_FACTOR = 2.20                 # Fallback: TP2 = Entry + 1.2*(TP1-Entry)

# ====== ATR-Fallback (wenn S/R nicht verfügbar) ======
ATR_SL  = 2.8
TP1_ATR = 5.0
TP2_ATR = 8.0
TP3_ATR = 14
...
REQUIRE_VOL_SPIKE  = False       # Vol-Spike nicht Pflicht, nur Bonus
...
# ====== Anzeige-Settings ======
COMPACT_SIGNALS = False

# ====== Early-STRIKT (optionale Frühwarnung) ======
...
# Micro-Fib (5m) für ersten Pullback (Early-STRIKT)
...
HEATMAP_BONUS_STRONG: int = 4         # Bonuspunkte bei starker nahe Wall
HEATMAP_BONUS_WEAK:   int = 2         # Bonuspunkte bei schwächerer nahe Wall

# ====== Score-Bonus Settings ======
TIME_BONUS = 5    # Bonuspunkte bei Signalen in UTC 0–2 Uhr oder 14–17 Uhr
SAFE_BONUS = 5    # Bonuspunkte, wenn Setup ein Safe Entry (Fib 0.5–0.618) ist
...
early_last_signal: Dict[str, float] = {}   # separater Cooldown für Early-STRIKT
last_scan_report: Dict[str, Any] = {"ts": None, "symbols": {}}
daily_mute: Dict[str, float] = {}          # key="SYM" -> Zeitstempel letztes Hauptsignal
...
def vol_sma(v: pd.Series, length: int = 20):
...
def bearish_engulf(o, h, l, c) -> bool:
    return (c.iloc[-1] < o.iloc[-1]) and (o.iloc[-1] >= h.iloc[-2]) and (c.iloc[-1] <= h.iloc[-2])
...
def analyze_trigger(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # MACD (12, 26, 9) – Histogramm für Momentum
    macd_df = ta.macd(df.close, 12, 26, 9)
    ...
    if adx_df is not None and not adx_df.empty:
        df["adx"] = adx_df["ADX_14"] if "ADX_14" in adx_df else adx_df.iloc[:, 0]
    else:
        df["adx"] = pd.Series([0] * len(df))

    o, h, l, c, v = df.open, df.high, df.low, df.close, df.volume
    long_fast  = (c.iloc[-1] > df.ema50.iloc[-1] > df.ema100.iloc[-1]) and (df.rsi.iloc[-1] <= 65) and (df.macd_hist.iloc[-1] > 0)
    short_fast = (c.iloc[-1] < df.ema50.iloc[-1] < df.ema100.iloc[-1]) and (df.rsi.iloc[-1] >= 35) and (df.macd_hist.iloc[-1] < 0)
...
# ====== Safe-Entry (letzte Impulsbewegung auf SAFE_ENTRY_TF) ======
...
def build_checklist_for_dir(direction: str, trig: Dict[str, Any], up_all: bool, dn_all: bool,
    ...
    # ADX Trendstärke (KO)
    if trig["adx"] >= 20:
        ok.append(f"ADX≥20 ({trig['adx']:.1f})")
    else:
        return (False, ok, [f"ADX<20 ({trig['adx']:.1f})"])

    # HTF-Alignment 15m/1h (KO)
    if (up_all if direction == "LONG" else dn_all):
        ok.append("HTF align (15m/1h)")
    else:
        return (False, ok, ["HTF nicht aligned (15m/1h)"])

    # Bias (Engulf oder EMA-Stack als Momentum, KO)
    ...
    # Volumen-Spike (Pflicht oder Bonus je nach Einstellung)
    if REQUIRE_VOL_SPIKE:
        if trig["vol_ok"]:
            ok.append(f"Vol>{VOL_SPIKE_FACTOR:.2f}×MA20 (Pflicht)")
        else:
            return (False, ok, [f"kein Vol-Spike (≥{VOL_SPIKE_FACTOR:.2f}× erforderlich)"])
    else:
        if trig["vol_ok"]:
            ok.append(f"Vol>{VOL_SPIKE_FACTOR:.2f}×MA20 (Bonus)")
        else:
    ...
    # RSI-Hinweis (Warnung bei extremen Werten)
    if direction == "LONG":
        if trig["rsi"] > 70:
...
# ====== Micro-Fib (Early-STRIKT) ======
def _pivot_idxs(vals: List[float], L: int, R: int, is_high: bool) -> List[int]:
    out = []
    n = len(vals)
    for i in range(L, n - R):
        left = vals[i-L:i]
        right = vals[i+1:i+1+R]
        if is_high and all(vals[i] > x for x in left + right):
            out.append(i)
        if (not is_high) and all(vals[i] < x for x in left + right):
            out.append(i)
    return out
...
async def send_signal(symbol: str, tf: str, direction: str,
    ...
    if COMPACT_SIGNALS:
        ...
        # Detaillierte Signaldarstellung mit Qualitäts-Checkliste
        checks_line = ""
        if checklist_ok:
...
async def send_mode_banner():
    ...
    text = (
        "🛡 *Scanner gestartet – MODUS: LOCKER (≥80%) + S/R-Ziele*\n"
        ...
        "• TP1=15m, TP2=1h, TP3=4h (sofern vorhanden). Fallback: ATR (entfernte TP2/TP3)\n"
...
async def scan_once():
    ...
    for sym in SYMBOLS:
        try:
            # 24h-Mute Check (nur Hauptsignal)
            ...
            # Trigger-Analyse (z.B. 5m)
            df_trigger = fetch_df(sym, TF_TRIGGER)
            trig = analyze_trigger(df_trigger)

            # ====== HEATMAP START ======
            heatmap_info = None
            if HEATMAP_ENABLED:
                try:
                    ob = ex.fetch_order_book(sym, limit=HEATMAP_LIMIT)
                    px = float(trig["price"])
            ...
            # === Early-STRIKT (optionale Vorwarnung, unabhängig von 24h-Mute) ===
            if EARLY_WARN_ENABLED and EARLY_STRICT_MODE:
                ew_dir, ew_score, ew_cons, ew_level = early_confluence(df_trigger)
                if ew_dir and (ew_score >= 4):  # mindestens 4 von 5 Kriterien erfüllt
            ...
            # Safe-Entry Analyse (Fib-Zone auf 15m, abgeschlossene Kerze)
            ...
            if imp is not None and len(df_safe_closed) >= 2:
                ...
                v_prev = float(v.iloc[-2]) if len(v) > 1 else 0.0
            ...
            # Trendfilter Hauptsignal – 15m & 1h in Trendrichtung?
            up_all, dn_all = True, True
            for tf in TF_FILTERS:
                df_tf = fetch_df(sym, tf)
                up, dn, _ = compute_trend_ok(df_tf)
            ...
            # Mögliche Signal-Richtungen prüfen
            passes = []
            for direction in ("LONG", "SHORT"):
                passed, ok_tags, warn_tags = build_checklist_for_dir(direction, trig, up_all, dn_all, safe_ok_long, safe_ok_short)
                if passed:
                    ...
                    # Bonuspunkte für Safe-Entry und Zeitfenster
                    safe_bonus = 0
                    if (direction == "LONG" and safe_ok_long) or (direction == "SHORT" and safe_ok_short):
                        safe_bonus = SAFE_BONUS
                    time_bonus = 0
                    utc_hour = datetime.now(timezone.utc).hour
                    if (0 <= utc_hour <= 2) or (14 <= utc_hour <= 17):
                        time_bonus = TIME_BONUS
                        ok_tags.append("Zeitbonus")
                    prob += safe_bonus + time_bonus

                    # ====== HEATMAP START ======
                    hm_bonus = 0
                    if HEATMAP_ENABLED and HEATMAP_USE_IN_ANALYSIS and heatmap_info and "error" not in heatmap_info:
                        entry_ref = float(trig["price"])
                        if direction == "LONG":
            ...
            if passes:
                # Richtung mit höchster (ggf. angepasster) Wahrscheinlichkeit wählen
...
async def test():
    text = (
        "✅ Test: Bot & Telegram OK — Mode: *LOCKER (≥80%) + S/R*, "
