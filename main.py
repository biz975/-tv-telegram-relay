...
daily_mute: Dict[str, float] = {}# Autonomous MEXC scanner → Telegram (FastAPI + Scheduler)
# MODE: Locker, aber ≥70% — Safe-Entry optional (mit Volumen-Bestätigung), Volumen-Spike nur Bonus, PROB_MIN=70
# S/R-Ziele (15m/1h/4h), Scan alle 15 Minuten
# Early-STRIKT (BOS+Retest, EMA-Stack, Micro-Fib) bleibt aktiv
...
async def send_mode_banner():
    banner = "🤖 *MEXC Scanner gestartet*\nMode: Locker (≥70% + S/R + Early Strict + Heatmap)"
...
async def scan_once():
    ...
    for sym in SYMBOLS:
        try:
            ...
            time.sleep(ex.rateLimit/1000)# ====== Weitere TA/Analyse-Funktionen ======
def bullish_engulf(o, h, l, c) -> bool:
    return (c.iloc[-1] > o.iloc[-1]) and (o.iloc[-1] <= l.iloc[-2]) and (c.iloc[-1] >= h.iloc[-2])
...
@app.get("/")
async def root():
    return {"ok": True, "mode": "locker_sr_min70"}
...
@app.get("/test")
async def test():
    await bot.send_message(chat_id=TG_CHAT_ID, text="✅ Test: Bot läuft mit Heatmap", parse_mode="Markdown")
    return {"ok": True}# ====== Runner / API ======
async def runner():
    sched = AsyncIOScheduler()
    sched.add_job(scan_once, "interval", seconds=SCAN_INTERVAL_S, next_run_time=datetime.now(timezone.utc))
    sched.start()
